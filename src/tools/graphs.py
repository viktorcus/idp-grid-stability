...
def line_loading(line_list, date, results_dir, scenario):
...
def graph_p_mw(date, net_element, results_dir, scenario):
    ...
    ax.set(xlabel=f'time step ({date})', ylabel='power [MW]', title=f'{scenario}Power Flow ({net_element})')
    plt.show()
    ax.figure.savefig(f'..\\results\\{results_dir}\\res_{net_element}\\p_mw.png')

def graph_battery_soc(net, date, results_dir, scenario):
